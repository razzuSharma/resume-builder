...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 transition-colors duration-300">
          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="px-4 py-2 text-gray-600 dark:text-gray-300 hover:text-teal-700 dark:hover:text-teal-400 font-medium rounded-lg hover:bg-teal-50 dark:hover:bg-gray-800 transition-all duration-200"
              >
                {link.label}
              </Link>
            ))}
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden absolute top-full left-0 right-0 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 shadow-lg transition-all duration-300 ${
          isMenuOpen
        ...
        <div className="px-4 py-4 space-y-2">
          {navLinks.map((link) => (
            <Link
              ...
              className="block px-4 py-3 text-gray-700 dark:text-gray-200 hover:text-teal-700 dark:hover:text-teal-400 font-medium rounded-xl hover:bg-teal-50 dark:hover:bg-gray-800 transition-all duration-200"
            >
              {link.label}
            </Link>
          ))}
